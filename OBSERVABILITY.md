...
| `recipe` | eine je Zug, aus dem ein Rezept werden konnte (WB-337). Label `saved`, `dropped` (sie wollte keines) oder `empty` (es blieb keine bestätigte Zutat übrig), Metadaten mit `dish`, `recipe_name`, `recipe_id` und `items`. Kein Score: „ein Rezept ist entstanden“ ist keine Bewertung eines Vorschlags und hat in keinem Mittelwert etwas zu suchen. **Ein abgelöster Zug bekommt keine** (WB-403): wer ein anderes Rezept wählt, ersetzt den Zug an seiner Stelle — aus ihm ist kein Rezept geworden, weil sie ein anderes wollte, und nicht, weil keine Zutat bestätigt wurde. Ein `empty` dafür schöbe bei jedem Rezeptwechsel einen Fehlschlag in diese Verteilung. Die Entscheidungen darin zählen weiter: `mapping_precision` und `suggestion` sind Aussagen über Vorschläge, die ein Mensch wirklich beurteilt hat. |
