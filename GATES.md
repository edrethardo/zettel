...
Sieben Fragen, zwölf Checks: private Angaben in der Historie (`git log -S`)
und im Arbeitsbaum · LICENSE · Testzahl und Check-Zahl in allen Dokumenten,
die in der Gegenwart sprechen · jeder relative Verweis zeigt auf eine
vorhandene Datei · Platzhalter nur im Post-Entwurf · die Modellbehauptung
gegen die Messdaten: solange kein Nemotron-Lauf des Wochenplaners in `evals/`
liegt, muss der Vorbehalt im Post stehen — liegt einer, muss er weg · und
seit WB-604 **fremde Rezepte**: keine echte Chefkoch-Antwort im Arbeitsbaum
und in keiner Fassung der Historie.

Der zwölfte Check hat einen Anlass wie die anderen. Zwei vollständige
Chefkoch-Antworten lagen als Test-Fixtures im Repo und waren über
`raw.githubusercontent.com` abrufbar — zwölf Rezepte mit Titeln,
Bewertungen und den Klarnamen ihrer Einsteller, dazu ein Rezept mit 23
Zutaten samt Mengen und Zubereitung. Die eingecheckten Fixtures sind seither
**erfunden**: ein Gericht, das es nicht gibt, mit erfundenen IDs aus dem
Block `9999…`. Der Check lässt genau diesen Block durch; eine echte ID, der
Chefkoch-Bildserver oder eine der drei IDs, die einmal hier lagen, machen ihn
rot.

**Im Arbeitsrepo sind vier Checks rot, und das bleibt so** — die Historie
trägt Hostname, Benutzername, Tailnet-Adresse und (bis WB-604) die beiden
echten Chefkoch-Fixtures. Das ist kein Fehler im
...
einen Maschine und seit WB-604 jede Fassung der beiden echten
Chefkoch-Fixtures. Die Liste
liegt neben dem Repo, nicht darin — sie enthält ja genau die Strings. **Im
gefilterten Klon läuft dasselbe Gate noch einmal, und dort müssen alle zwölf
grün sein** (Stand 17.09.2026: sind sie). Gepusht wird der Klon, nie das
Arbeitsrepo.
...
produktive Weg ist aber Chat -> Chefkoch -> Vorschlag -> Korb. Ein Chat-Zug
zu „alles für Bolognese" legt beim „Ja" **200 g** in den Korb und nicht die
