...
#: Der Präfix von vor WB-401. Geschrieben wird er nicht mehr, GEFUNDEN schon:
#: die Stände, die vor der Umbenennung entstanden sind, liegen im selben
#: Verzeichnis. Ein Muster, das nur den neuen Namen kennt, sähe sie nicht —
#: sie würden nie mehr abgeräumt und wären in `staende()` unsichtbar, also
#: genau dann nicht da, wenn jemand nach einer Sicherung sucht.
ALT_PRAEFIX = "picknick-"

#: Erkennt genau die Dateien, die `sichern()` anlegt (oder vor WB-401 angelegt
#: hat). Bewusst streng: im Sicherungsverzeichnis wird gelöscht, und dabei soll
#: nichts abgeräumt werden, was jemand von Hand dort abgelegt hat. Der
#: Zeitstempel steht in Gruppe 1 — danach wird sortiert, siehe `staende()`.
STAND_MUSTER = re.compile(
    r"^(?:" + re.escape(PRAEFIX) + r"|" + re.escape(ALT_PRAEFIX) + r")"
...
def staende(ziel_dir: str | Path) -> list[Path]:
    ...
    Sortiert wird über den Zeitstempel IM Dateinamen und nicht über die mtime:
    der Name trägt den Zeitpunkt des Laufs, die mtime überlebt ein Kopieren des
    Verzeichnisses nicht. Der ISO-Stempel sortiert lexikografisch wie
    chronologisch.

    Und ausdrücklich nicht über den ganzen Namen: seit WB-401 stehen im selben
    Verzeichnis zwei Präfixe, `zettel-` und das alte `picknick-`. Über den
    Namen sortiert stünde jeder alte Stand vor jedem neuen, egal wie jung er
    ist — und `abraeumen()` löschte dann den falschen.
    """
    ziel = Path(ziel_dir)
    if not ziel.is_dir():
        return []
    treffer = ((p, STAND_MUSTER.match(p.name)) for p in ziel.iterdir())
    return [p for p, _ in sorted(((p, m) for p, m in treffer if m),
                                 key=lambda paar: paar[1].group(1))]
...
#: Modul über `zettel.assistant` das ganze Chat-Paket samt `openai` herein,
...
def modellstand(con: sqlite3.Connection) -> dict:
    ...
    * `fehler` — die Konfiguration ist so nicht benutzbar. Das ist die einzige
    ...
    try:
        ...
        from zettel.llm.client import endpunkt_aus_umgebung
