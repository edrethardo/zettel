...
Spec 7.1 `zettel.path`), weil sich sonst später keine Auswertung mehr
trennen lässt:
...
Der Rezeptweg erzeugt nur den CHAIN-Span, mit `zettel.path = "recipe"`.
...
ohnehin am Span (`input.value`) und am `zettel.path` daneben.
...
class Chat:
    ...
    def _span_abschluss(self, span, ergebnis: Ergebnis,
        ...
        obs.setze(span, {
            ...
            "zettel.chat_message_id": ergebnis.chat_message_id,
            "zettel.terms": len(ergebnis.begriffe),
            "zettel.products": ergebnis.n_produkte,
            "zettel.free_text": ergebnis.n_freitext,
            # Wie oft das Modell eine ID nannte, die ihm nie vorgelegt wurde.
            # Die interessanteste Zahl von Stufe 3 (Spec 6).
            "zettel.rejected": len(ergebnis.verworfen),
            "zettel.recipes": ", ".join(ergebnis.rezepte) or None,
            "zettel.weakest_term": schwach,
            "zettel.weakest_rank": rang,
            ...
            "zettel.dish": ergebnis.gericht,
            "zettel.dish_recipe": ergebnis.quelle_name,
            "zettel.dish_url": ergebnis.quelle_url,
            ...
            "zettel.dish_requested": bool(ergebnis.abruf) or None,
            "zettel.dish_fetch": ergebnis.abruf,
            ...
            # Tipp mehr?
            "zettel.fanout_category": ergebnis.kategorie,
            "zettel.fanout_varieties": len(ergebnis.sorten) or None,
            "zettel.fanout_source": ergebnis.sorten_herkunft,
            # Wie `rejected` bei den Produkt-IDs: wie oft das Modell eine
            # Kategorie nannte, die ihm nie vorgelegt wurde.
            "zettel.fanout_rejected": ergebnis.sorten_verworfen,
            "zettel.varieties_chosen":
            ...
            "zettel.dish_draft": ergebnis.entwurf,
            "zettel.dish_items": ergebnis.entwurf_zutaten or None,
            ...
            "zettel.rest": ergebnis.rest,
            "zettel.rest_added": ergebnis.rest_angehaengt,
            ...
            "zettel.dish_switch": ergebnis.gewechselt,
        })
    ...
    def _sorten_suchen(self, con, kategorie: str,
        ...
        for sorte in sorten:
            with obs.retriever("catalog.search", suchbegriffe=[sorte]) as such:
                ...
                obs.setze(such, {"zettel.qty": 1,
                                 "zettel.candidates_kept": len(aufgehoben),
                                 # Woher die Kandidaten kamen. Ohne das sähe
                                 # der Span aus wie eine Suche, die zufällig
                                 # keinen Rang hat.
                                 "zettel.category": f"{kategorie} > {sorte}"})
    ...
    def _suchen(self, con, begriffe: list[dict]) -> list[dict]:
        ...
        for b in begriffe:
            ...
            with obs.retriever("catalog.search", suchbegriffe=kette) as such:
                ...
                obs.dokumente(such, kandidaten)
                obs.setze(such, {"zettel.qty": b["menge"],
                                 "zettel.candidates_kept": len(aufgehoben)})
