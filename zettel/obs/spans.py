...
die Zusammenfassung: `zettel.weakest_term` und `zettel.weakest_rank` — der
...
`zettel.weakest_rank` sortiert, sieht die Retrieval-Probleme zuerst.

**Die Attributnamen unter `zettel.` sind englisch.** Der Code dieses
...
wechselt, wird falsch getippt. Spec 7.1 gibt mit `zettel.path` denselben
Weg vor.
...
from zettel.obs.otel import tracer
...
def retriever(name: str, *, suchbegriffe: list[str]):
    ...
    genauesten zum allgemeinsten) und zusätzlich als `zettel.search_terms`
    ...
    with tracer().start_as_current_span(name) as span:
        ...
        setze(span, {
            ...
            "zettel.term": kette[0] if kette else None,
            "zettel.search_terms": ", ".join(kette) or None,
        })
        yield span
...
def dokumente(span, treffer: list[dict]) -> None:
    ...
    setze(span, {
        ...
        "zettel.rank_top": rang_top(treffer),
    })
