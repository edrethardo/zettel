...
**Kein Test geht ins Netz.** Chefkoch ist die ERFUNDENE Zwirbel-Suche aus
`tests/fixtures/` samt dem Detail des Rezepts, das dabei gewinnt; das Detail
einer ALTERNATIVE gibt es dort nicht — es wird hier aus derselben Fixture
...
#: Die erste Alternative darunter — „mit Huhn", 4,85 aus 33 Stimmen.
ZWIRBEL_HUHN = "9999000000000002"
...
class FakeHTTP:
    """Chefkoch, erfunden. Merkt sich JEDE URL — daran hängt die Zusage
    „die Wahl löst keinen zweiten Suchabruf aus"."""
...
def _alternativ_detail(rezept_id: str, titel: str) -> dict:
    """Das Detail einer Alternative — aus der Fixture nachgebaut.
    ...
    detail["ingredientGroups"] = [{
        "header": "",
        "ingredients": [
            {"name": "Pastinake", "amount": 1.0, "unit": "Stück",
             "usageInfo": ""},
            {"name": "Speckwürfel", "amount": 250.0, "unit": "g",
             "usageInfo": ""},
        ],
    }]
    return detail
...
def _zwirbel_geholt(pfad) -> int:
    """Holt Zwirbel aus der Fixture — Suche UND Detail, wie im echten Abruf."""
    con = db.connect(pfad)
    try:
...
def _antworten(pfad, n: int = 2):
    ...
    paar = [_extract((("Pastinake",), 1), (("Speckwürfel", "Speck"), 1)),
...
def _shop(pfad, tmp_path, *, waehler=None, antworten=None, box=None):
    ...
    """
    http = FakeHTTP({f"/v2/recipes/{ZWIRBEL_HUHN}":
                     _alternativ_detail(ZWIRBEL_HUHN, "Zwirbeltopf mit Huhn")})
...
def test_zur_wahl_stehen_sechs_ohne_plus_und_bestgewichtet_zuerst(datei):
    ...
    assert wahl[0]["rezept_id"] == ZWIRBEL, "die Vorauswahl steht oben"
...
def test_ein_chefkoch_zug_bietet_alternativen_mit_zahlen_an(datei, tmp_path):
    ...
    kennt die Suchantwort nur die ARBEITSZEIT. Der Zwirbeltopf steht dort
    mit 90
    ...
    assert "5 andere Rezepte zu „Zwirbel“" in _eine_zeile(karte)
    ...
    assert "Zwirbeltopf mit Huhn" in karte
...
def test_der_rezeptweg_bietet_nichts_an(datei, tmp_path):
    ...
    try:
        ...
        recipes.zutat_hinzufuegen(con, rid,
                                  product_id=_pid(datei, "Speckwürfel"))
    finally:
        con.close()
...
def test_die_wahl_fuehrt_in_den_normalen_ablauf(datei, tmp_path):
    ...
    assert waehler.gewaehlt == [ZWIRBEL_HUHN]
    ...
    try:
        ...
        con.close()
    assert saetze == ["alles für Zwirbel, und Klopapier"] * 2
...
def test_die_wahl_loest_keinen_zweiten_suchabruf_aus(datei, tmp_path):
    ...
    client, http, _ = _shop(datei, tmp_path)
    client.post("/chat", data={"satz": "alles für Zwirbel"}, headers=HTMX)
    _wechsel(client, _zug_id(datei), ZWIRBEL_HUHN)
    ...
    assert http.geholt[0].endswith(f"/recipes/{ZWIRBEL_HUHN}")
...
def test_zurueckwechseln_kostet_gar_keine_anfrage(datei, tmp_path):
    ...
    client.post("/chat", data={"satz": "alles für Zwirbel"}, headers=HTMX)
    _wechsel(client, _zug_id(datei), ZWIRBEL_HUHN)
    _wechsel(client, _zug_id(datei), ZWIRBEL)

    assert _rezept_des_gerichts(datei) == ZWIRBEL
    # Nur die eine Anfrage für das Huhn; der Rückweg fasst das Netz nicht an.
    assert len(http.geholt) == 1
    assert waehler.gewaehlt == [ZWIRBEL_HUHN]
...
def test_ohne_wahl_bleibt_alles_wie_es_war(datei, tmp_path):
    ...
    assert _rezept_des_gerichts(datei) == ZWIRBEL
    seite = client.get("/chat").text
    assert "Zwirbeltopf mit Rinderbrühe" in seite
    con = db.connect(datei)
    try:
        n = con.execute("SELECT count(*) AS n FROM chat_suggestion"
                        ).fetchone()["n"]
...
def test_der_wechsel_steht_am_zug(datei, tmp_path):
    ...
    try:
        ...
        ergebnis = agent.turn(con, "alles für Zwirbel",
                              gewechselt="Zwirbeltopf mit Huhn")
    finally:
        con.close()
    assert ergebnis.gewechselt == "Zwirbeltopf mit Huhn"
...
def test_der_altbestand_laesst_sich_von_hand_nachholen(datei):
    ...
    try:
        ...
        con.close()
    assert [o["query"] for o in offen] == ["Zwirbel"]
