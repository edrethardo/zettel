...
**Kein Test geht ins Netz.** Chefkoch ist der ERFUNDENE Zwirbeltopf aus
...
bloss, DASS das Rezept dasteht, sondern in welcher REIHENFOLGE: der
Zwirbeltopf sind neuneinhalb Stunden und die Käse-Lauch-Suppe
fünfunddreissig Minuten, und das ist die Zahl, die entscheidet. Sie muss vor
der Zutatenliste stehen und darf nie eingeklappt sein.
"""
...
def datei(vorlagen, tmp_path):
    """Katalog plus die drei Produkte, die der Zwirbel-Zug findet."""
...
def _zwirbel_geholt(pfad):
    """Holt den Zwirbeltopf aus der Fixture. Gibt die recipe_id."""
    con = db.connect(pfad)
    try:
        lauf.hole_eines(con, FakeHTTP({"/v2/recipes?": SUCHE,
                                       f"/v2/recipes/{ZWIRBEL}": REZEPT}),
                        "Zwirbel", pause_s=0, schreib=lambda _: None)
...
def _zwirbel_zug(pfad, tmp_path):
    ...
    antwort = client.post("/chat", data={"satz": "alles für Zwirbel"},
                          headers=HTMX)
...
def test_ein_chefkoch_zug_zeigt_das_rezept(datei, tmp_path):
    ...
    assert "Zwirbeltopf mit Rinderbrühe" in karte
    # Die Zutatenliste, wie die Quelle sie schreibt — mit Menge und Einheit.
    assert "Rinderknochen" in karte and "Liebstöckel" in karte
    ...
    # Und die Gruppen, die die Quelle setzt („Für den Sud").
    assert "Für den Sud" in karte
...
def test_die_gesamtzeit_steht_ganz_oben_und_als_eine_zahl(datei, tmp_path):
    ...
    Der Zwirbeltopf sind 90 + 480 Minuten. „9½ Stunden" ist die Auskunft;
    „prep 90, cook 480" ist eine Datenbankzeile.
    """
    ...
    assert karte.index("9½ Stunden") < karte.index("Rinderknochen")
...
def test_die_katalogdeckung_steht_vor_der_zutatenliste(datei, tmp_path):
    ...
    assert karte.index("deckung") < karte.index("Rinderknochen")
...
def test_die_zubereitung_ist_da_aber_nicht_aufgeklappt(datei, tmp_path):
    ...
    Die Zubereitung des Zwirbeltopfs sind 1.831 Zeichen, die des echten
    Rezepts, an dem das gemessen wurde, 3.924. Ein `<details>` allein
    versteckt nur, was trotzdem übertragen wurde; hier wird sie beim
    Aufklappen geholt.
    """
    client, recipe_id = _zwirbel_zug(datei, tmp_path)
...
def test_der_zug_traegt_die_karte_auch_beim_teiltausch(datei, tmp_path):
    ...
    assert "9½ Stunden" in karte and "Rinderknochen" in karte
...
@pytest.mark.parametrize("minuten,satz", [
    ...
    (570, "9½ Stunden"),                 # der Zwirbeltopf: 90 + 480
])
