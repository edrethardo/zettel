...
picknick-crawl` und nicht nur in einer Tabelle, in die niemand schaut.
"""
...
USER_AGENT = "picknick/1.0 (private household catalogue; nightly, 1 req/1.5 s)"
...
def main(argv=None) -> int:
    ...
    p.add_argument("--db", default=None,
                   help=f"Datenbankdatei (Vorgabe: $PICKNICK_DB oder {db.DEFAULT_DB})")
    p.add_argument("--image-dir", default=None,
                   help="Bildverzeichnis (Vorgabe: $PICKNICK_IMAGE_DIR oder data/images)")
    ...
    import os
    db_path = args.db or os.environ.get("PICKNICK_DB") or db.DEFAULT_DB
    image_dir = (args.image_dir or os.environ.get("PICKNICK_IMAGE_DIR")
                 or "data/images")
    if args.begriff:
