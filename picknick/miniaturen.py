...
* **Nicht im Request-Pfad.** Umrechnen kostet ~11 ms je Bild; das gehört nicht
  ...
  (`picknick.scrapers.nachtlauf`) und von Hand über `python -m
  picknick.miniaturen`.
...
#: (`PICKNICK_IMAGE_DIR`) und ein Test mit eigenem `tmp_path` nichts zusätzlich
#: umbiegen muss.
UNTERORDNER = "mini"
...
def main(argv=None) -> int:
    ...
    p.add_argument("--image-dir", default=None,
                   help="Bildverzeichnis (Vorgabe: $PICKNICK_IMAGE_DIR "
                        "oder data/images)")
